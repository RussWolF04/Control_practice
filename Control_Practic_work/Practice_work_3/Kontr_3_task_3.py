...
class Tomato:
    '''Создали класс Томато'''

    # Стадии созревания помидора
    states = {0: 'Рассада', 1: 'Цветение', 2: 'Зеленый_Плод', 3: 'Спелый_Помидор'}
...
class Gardener:
    ...
    def work(self):
        print('Садовник начинает работу на участке с помидорами')
        self._plant.grow_all()
        print('Садовник окончил работу на участке с помидорами')

    # Собираем урожай
    def harvest(self):
        print('Садовник собирает помидоры...')
...
когда плоды будут зрелыми зелеными, а
затем им будет позволено созреть на виноградной лозе.
Это предотвращает расщепление или образование синяков
и позволяет в определенной степени контролировать процесс созревания.''')
